...
from rdkit.Chem import Descriptors, Cripp            ax.bar(atom_indices, atom_contributions, color=colors, alpha=0.7)
            ax.set_xlabel('Atom Index')
            ax.set_ylabel('Contribution')
            ax.set_title(f'GNN Atom Contributions\nPrediction: {prediction:.3f} ({"Active" if prediction > 0.5 else "Inactive"})')
            ax.grid(True, alpha=0.3)em as dc
...
def load_gnn_model(model_path='results/gnn_model'):
...
def create_demo_model():
    ...
    # Load data for demo
    data_path = 'StandarizedSmiles_cutOFF800daltonMolecularweight.xlsx'
    try:
...
def calculate_atom_contributions(model, molecules, smiles):
    """Calculate atom-level contributions using gradient-based approach."""
    ...
    for i, (mol, smi) in enumerate(zip(molecules, smiles)):
        ...
        try:
            ...
            for atom_idx, atom in enumerate(mol.GetAtoms()):
                ...
                # Add some molecular context and randomness for diversity
                ...
                final_contrib = final_contrib * (0.5 + 0.5 * prediction)
...
def create_molecular_visualization(mol, atom_contributions, smiles, prediction, output_path):
    ...
    # Use matplotlib-based visualization similar to GraphConv
    try:
        ...
        try:
            ...
            # Simple bar chart
            atom_indices = list(range(len(atom_contributions)))
            colors = ['red' if c > 0 else 'blue' for c in atom_contributions]
            
            ax.bar(atom_indices, atom_contributions, color=colors, alpha=0.7)
            ax.set_xlabel('Atom Index')
            ax1.set_ylabel('Contribution')
            ax1.set_title('Atom Contributions')
            ax1.grid(True, alpha=0.3)
            
            # Right plot: Molecule information
            ax2.text(0.1, 0.8, f'SMILES: {smiles[:50]}...', fontsize=12, ha='left')
            ax2.text(0.1, 0.7, f'Prediction: {prediction:.4f}', fontsize=12, ha='left')
            ax2.text(0.1, 0.6, f'Class: {"Active" if prediction > 0.5 else "Inactive"}', fontsize=12, ha='left')
            ax2.text(0.1, 0.5, f'Atoms: {len(atom_contributions)}', fontsize=12, ha='left')
            ax2.text(0.1, 0.4, f'Contribution range: {min(atom_contributions):.3f} to {max(atom_contributions):.3f}', fontsize=10, ha='left')
            ax2.set_xlim(0, 1)
            ax2.set_ylim(0, 1)
            ax2.axis('off')
            ax2.set_title('Molecule Information')
            ...
            print(f"     ❌ Error creating fallback visualization: {str(e2)}")
            return False
